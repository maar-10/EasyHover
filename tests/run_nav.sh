#!/usr/bin/env bash
# nav suite, in its OWN CraftOS instance.
#
#   bash tests/run_nav.sh
#
# Separate from the other suites on purpose: three roles now have a lib/config.lua, so a shared
# Lua state would make require("lib.geo") resolve against whichever role loaded first.
